  ...
  //常规用法
    ...
    Attendance.initColumn = function () {
        return [[
            ...
            {field: 'name', sort: true, title: '考勤学生'},
            {field: 'no', sort: true, title: '考勤学号'},
            {field: 'time', sort: true, title: '考勤时间'},
            {field: 'type', sort: true, title: '考勤类型'},
    ...
    Attendance.search = function () {
        ...
        queryData['deptId'] = $("#classesId").val();   
    ...
    Attendance.delete1 = function (data) {
        admin.putTempData('formOk', false);
        top.layui.admin.open({
            type: 2,
            area : [ '650px', '600px' ], // 宽高
            title: '查看考勤位置',
            content: Feng.ctxPath + '/attendance/map?attendId=' + data.attendId,
            end: function () {
                admin.getTempData('formOk') && table.reload(Attendance.tableId);
            }
        });
    ...
    var tableResult = table.render({
        ...
        url: Feng.ctxPath + '/attendance/list1?roomId='+ Feng.getUrlParam("roomId"),
        page: true,
        height: "full-158",
        cellMinWidth: 100,
    ...
    table.on('tool(' + Attendance.tableId + ')', function (obj) {
        ...
        } else if (layEvent === 'delete1') {
            Attendance.delete1(data);
        }
    });
});
