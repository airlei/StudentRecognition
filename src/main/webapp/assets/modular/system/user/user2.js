layui.use(['layer', 'form', 'table', 'ztree', 'laydate', 'admin', 'ax','upload'], function () {
    ...
    MgrUser.initColumn = function () {
        return [[
               ...
               {field: 'account', sort: false, title: '账号1'},
               {field: 'email', sort: false, title: '学号'},
               {field: 'name', sort: false, title: '姓名'},
               {field: 'sex', sort: false, title: '性别', templet: function(d){
                   if (d.sex =="M") {  // 自定义内容
                       return "男";
                   }else{
                       return "女";
    ...
    MgrUser.onEditUser = function (data) {
        ...
        top.layui.admin.open({
            type: 2,
            title: '编辑学生',
            content: Feng.ctxPath + '/mgr/user_edit2?userId=' + data.userId,
            end: function () {
                admin.getTempData('formOk') && table.reload(MgrUser.tableId);
            }
        });
