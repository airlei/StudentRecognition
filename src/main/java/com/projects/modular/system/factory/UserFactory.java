...
public class UserFactory {
	...
	public static User createUser1(UserDto userDto, String md5Password, String salt) {
		if (userDto == null) {
			return null;
		} else {
			User user = new User();
			...
			user.setRoleId("1323546363237978113");
			user.setSalt(salt);
			return user;
		}
	}
	public static User createUser2(UserDto userDto, String md5Password, String salt) {
		if (userDto == null) {
			return null;
		} else {
			User user = new User();
			...
			user.setPassword(md5Password);
			user.setDeptId(userDto.getDeptId());
			user.setRoleId("1359012972354043905");
			user.setSalt(salt);
			return user;
		}
	}
